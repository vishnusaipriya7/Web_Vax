...
chrome.webRequest.onBeforeRequest.addListener(
  function(details) {
      ...
      if (vulnerabilities.length > 0) {
          ...
          if (highSeverityVulnerability) {
              notifyUser('Detected a high-risk request to ' + url.hostname, highSeverityVulnerability);
          }
      }
  },
  { urls: ["<all_urls>"] },
...
function logVulnerabilities(vulnerabilities, requestData) {
  vulnerabilities.forEach(vulnerability => {
      ...
      chrome.storage.localsetf({ threatStats: threatStats });
  });
}
